        ...
        ELSE 
            ...
            IF V_VALIDAALTERACAO >= 1 AND V_ALTERA >= 1 THEN 
                UPDATE TGFPRO SET 
                    ...
                    SOLCOMPRA, TEMRASTROLOTE, TIPLANCNOTA, USALOCAL, USALOTEDTFAB, USALOTEDTVAL, USOPROD, VENCOMPINDIV) =
                    (SELECT 
                        AD_CODEND, AD_DESCRICAOCOMEX, AD_IMPRIMEETIQUETA, AD_KITFANTASMA, AD_OBSNF, AD_QTDMINVENDA, AD_SEPARAPROD, CARACTERISTICAS, CODGRUPOPROD,
                        CODLOCALPADRAO, CODVOL, DESCRPROD, FABRICANTE, ORIGPROD, PRODUTONFE, REFERENCIA, 
                        SOLCOMPRA, TEMRASTROLOTE, TIPLANCNOTA, USALOCAL, USALOTEDTFAB, USALOTEDTVAL, USOPROD, VENCOMPINDIV
            ...
            IF V_VALIDAALTERACAO >= 1 AND V_ALTERAFIS >= 1 AND V_ALTERA >= 1 THEN 
                UPDATE TGFPRO SET 
                    (AD_GRAMAMLPMPF, AD_PMPF, CLASSUBTRIB, CODCTACTB, CODESPECST, CODEXNCM, CODIPI, CSTIPIENT, CSTIPISAI, 
                    GRUPOCOFINS, GRUPOCSSL, GRUPOICMS, GRUPOICMS2, GRUPOPIS, IDENTIMOB, NCM, TEMICMS, TEMIPICOMPRA, TEMIPIVENDA, TIPSUBST, UTILIMOB) =
                    (SELECT 
                        AD_GRAMAMLPMPF, AD_PMPF, CLASSUBTRIB, CODCTACTB, CODESPECST, NVL(CODEXNCM,0), CODIPI, CSTIPIENT, CSTIPISAI, GRUPOCOFINS, GRUPOCSSL, 
                        GRUPOICMS, GRUPOICMS2, GRUPOPIS, IDENTIMOB, NCM, TEMICMS, TEMIPICOMPRA, TEMIPIVENDA, TIPSUBST, UTILIMOB
                    FROM AD_TGFPROFIS
                    WHERE NUNICO = FIELD_NUNICO)
                WHERE CODPROD = V_CODPROD;
            ...
            IF V_VALIDAALTERACAO >= 1 AND V_ALTERA >= 1 THEN 
                UPDATE TGFPRO SET 
                    (AD_PALLET, AD_PESAGEM, AD_SEPARACAO, AD_VOLUMETRIA, ALTURA, DECQTD, DECVLR, ESPESSURA, ESTMIN, LARGURA, 
                    LEADTIME, M3, PERMCOMPPROD, PESOBRUTO, PESOLIQ, PRAZOVAL, QTDEMB, RASTRESTOQUE) =
                    (SELECT 
                        AD_PALLET, AD_PESAGEM, AD_SEPARACAO, AD_VOLUMETRIA, ALTURA, DECQTD, DECVLR, ESPESSURA, ESTMIN,
                        LARGURA, LEADTIME, M3, PERMCOMPPROD, PESOBRUTO, PESOLIQ, PRAZOVAL, QTDEMB, RASTRESTOQUE
                    FROM AD_TGFPROGER
                    WHERE NUNICO = FIELD_NUNICO)
                WHERE CODPROD = V_CODPROD;
